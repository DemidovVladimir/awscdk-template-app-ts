...
function getPackageManagerCommandPrefix(packageManager: javascript.NodePackageManager): string {
  switch (packageManager) {
    case javascript.NodePackageManager.NPM:
      return 'npm run'
    case javascript.NodePackageManager.YARN || javascript.NodePackageManager.YARN_CLASSIC || javascript.NodePackageManager.YARN_BERRY:
      return 'yarn'
...
export function checkActiveDeploymentStep(): JobStep {
  return {
    ...
    uses: 'DemidovVladimir/check-deployed-environment@v1',
    with: {
