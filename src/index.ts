...
import { AwsCdkTemplateTypeScriptAppOptions, DeployOptions, EnvironmentOptions } from './types';
import { getMajorNodeVersion } from './utils';

export * from './types';
export * as utils from './utils';

export class AwsCdkTemplateTypeScriptApp extends awscdk.AwsCdkTypeScriptApp {
  ...
  constructor(options: AwsCdkTemplateTypeScriptAppOptions) {
  ...
  private addDeployJobs() {
    ...
    if (this.deployOptions.environments.length === 0) {this.logger.warn('The project does not have any environment set, make sure this is desired setting');}
    ...
    if (taskToRunPreInstall) {jobDefinition.steps.push(steps.preInstallDependenciesStep(taskToRunPreInstall, this.checkActiveDeployment));}

    jobDefinition.steps.push(...(this.package.project as NodeProject).renderWorkflowSetup());

    if (this.checkActiveDeployment) {jobDefinition.steps.push(steps.checkActiveDeploymentStep());}

    jobDefinition.steps.push(...steps.setAwsCredentialsSteps(this.checkActiveDeployment, this.codeArtifactOptions?.authProvider));

    if (this.deployOptions.npmConfigEnvironment) {jobDefinition.steps.push(steps.setNpmConfig(this.deployOptions.npmConfigEnvironment, '${{ matrix.environment }}', this.checkActiveDeployment));}
